from panda3d.core import AntialiasAttrib, Camera, ConfigVariable, ConfigVariableBool, ConfigVariableDouble, ConfigVariableInt, DisplayInformation, GraphicsStateGuardian, Pixel, WindowProperties, loadPrcFile, loadPrcFileData, xel
import copy
import string
import os
import sys
...
class Options(OptionSpace):
    ...
    def __init__(self):
        ...
        self.wideByDefault = base.config.GetBool('wide-by-default', True)
    ...
    def load(self, file_path):
        ...
        try:
            ...
            self.tokenDict = {}
            num_tokens = [len(file_data)-1,len(file_data)][len(file_data) % 2 == 0]
            for i in xrange(0, num_tokens, 2):
                self.tokenDict[file_data[i]] = file_data[i+1]
    ...
    def config_to_options(self):
        ...
        self.fullscreen = base.config.GetBool('fullscreen', 0)
        if base.config.GetBool('want-water-reflection', 0):
            self.reflection = 2
            if base.config.GetBool('want-water-reflection-show-through-only', 0):
                self.reflection = 1
    ...
    def options_to_config(self):
        ...
        config_variable = ConfigVariableBool('want-water-reflection', 0)
        if self.reflection == 0:
            config_variable.setValue(0)
        else:
            config_variable.setValue(1)
        config_variable = ConfigVariableBool('want-water-reflection-show-through-only', 0)
...
class GameOptions(BorderFrame):
    ...
    def display_restart_dialog(self):
        ...
        self.restartDialog.setBin('gui-fixed', 2)
    ...
    def display_noteOnChange_dialog(self):
        ...
        self.noteOnChangeDialog.setBin('gui-fixed', 2)
    ...
    def display_stereoOption_dialog(self):
        ...
        self.stereoOptionDialog.setBin('gui-fixed', 2)
    ...
    def save_button_function(self):
        ...
        self.savedDialog.setBin('gui-fixed', 2)
