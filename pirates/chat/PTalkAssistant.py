...
class PTalkAssistant(TalkAssistant):
    ...
    def receiveOpenTalk(self, avatarId, accountId, accountName, message, scrubbed = 0, avatarName=None):
        error = None
        if not avatarName and avatarId:
            avatarName = self.findName(avatarId, 0)

        newMessage = TalkMessage(self.countMessage(), self.stampTime(), message, avatarId, avatarName, accountId, accountName, None, None, None, None, TALK_OPEN, None)
    ...
    def receivePartyTalk(self, fromAv, fromAC, avatarName, message, scrubbed = 0):
        error = None
        if not self.isThought(message):
            accountName = self.findName(fromAC, 1)
            newMessage = TalkMessage(self.countMessage(), self.stampTime(), message, fromAv, avatarName, fromAC, accountName, None, None, None, None, TALK_PARTY, None)
    ...
    def receiveOpenSpeedChat(self, msgType, messageIndex, senderId, name = None):
        ...
        newMessage = TalkMessage(self.countMessage(), self.stampTime(), message, senderId, name, None, None, None, None, None, None, messageType, None)
        self.historyComplete.append(newMessage)
        self.historyOpen.append(newMessage)
    ...
    def receivePartyMessage(self, message, senderId, senderName):
        error = None
        if not self.isThought(message):
            newMessage = TalkMessage(self.countMessage(), self.stampTime(), message, senderId, senderName, None, None, None, None, None, None, TALK_PARTY, None)
    ...
    def receiveShipPVPMessage(self, fromAv, fromAC, avatarName, teamName, message, scrubbed = 0):
        error = None
        if not self.isThought(message):
            accountName = self.findName(fromAC, 1)
            newMessage = TalkMessage(self.countMessage(), self.stampTime(), message, fromAv, avatarName, fromAC, None, None, None, None, None, TALK_PVP, teamName)
            reject = 0
            if fromAv:
                reject = self.addToHistoryDoId(newMessage, fromAv, scrubbed)
            ...
            if reject != 2:
                if fromAC:
                    self.addToHistoryDISLId(newMessage, fromAC)
